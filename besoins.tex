...
{Avec le besoin 2 la taille était sauvegardé comme le nombre total de pixel présent dans le l'image, de ce fait le choix du type d'algorithme à été décidé selon son extension. Or suite au besoin 4 le façon dont la taille est enregistré a changé, il aurait était peut-être plus intéressant de choisir le type d'algorithme selon le nombre de canaux présent dans sa taille.}
...
{}
{}
{Ce besoin n'est pas effectif, il se trouve dans l'algorithme. Ceci étant, il est possible de s'amuser avec(lol).)}
